...
def developer_node(state: AgencyState) -> Dict:
    ...
    - CRITICAL for Full-Stack imports: The backend is always run with "backend/" itself as the working directory/root (both in the test sandbox via `cd backend && uvicorn ...` and in Docker via `WORKDIR /app; COPY backend/ .`). Therefore Python imports between backend files MUST be relative to "backend/", NOT prefixed with "backend.". Example: a file at "backend/routes/tasks.py" must be imported from "backend/main.py" as `from routes.tasks import router`, NEVER `from backend.routes.tasks import router` — the latter raises `ModuleNotFoundError: No module named 'backend'` at runtime. Every package directory under "backend/" that is imported this way (e.g. "backend/routes/") MUST include an "__init__.py" file.
